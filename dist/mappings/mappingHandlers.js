...
async function handleBlock(block) {
    ...
    // Ultra-fast path: check if block has ANY non-system events
    // Most blocks on Lunes have only system.ExtrinsicSuccess + timestamp.Set
    let hasInteresting = false;
    for (let i = 0; i < events.length; i++) {
        const s = events[i].event.section;
        if (s !== 'system' && s !== 'timestamp' && s !== 'imOnline' && s !== 'grandpa' && s !== 'transactionPayment') {
            hasInteresting = true;
            break;
        }
    }
    // Only create block record for blocks with interesting activity
    // This saves ~95% of DB writes since most blocks are empty
    if (!hasInteresting)
        return;
    await (0, handlers_1.createBlock)(block);
    // Process only interesting events
    for (let idx = 0; idx < events.length; idx++) {
        const evt = events[idx];
        const section = evt.event.section;
        const method = evt.event.method;
        // Skip infrastructure events
        if (section === 'system' || section === 'timestamp' || section === 'imOnline'
            || section === 'grandpa' || section === 'transactionPayment')
            continue;
        (0, handlers_1.createEvent)(blockNumber, idx, evt);
        try {
            if (section === "balances" && method === "Transfer") {
                (0, handlers_1.createTransfer)(blockNumber, idx, evt, block.timestamp);
            }
            else if (section === "assets") {
                if (method === "Transferred") {
                    await (0, handlers_1.createAssetTransfer)(blockNumber, idx, evt);
                    const d = evt.event.data;
                    if (d && d.length >= 4) {
                        types_1.Transfer.create(Object.assign({ id: `${blockNumber}-${idx}`, fromId: d[1].toString(), toId: d[2].toString(), amount: BigInt(d[3].toString().replace(/,/g, '')), value: BigInt(d[3].toString().replace(/,/g, '')), blockId: blockNumber.toString(), blockNumber, eventIndex: idx }, (block.timestamp ? { timestamp: BigInt(block.timestamp.getTime()), date: block.timestamp } : {}))).save();
                    }
                }
                ...
                await (0, handlers_1.handleStakingEvent)(blockNumber, idx, evt);
            }
            else if (section === "contracts") {
                if (method === "ContractEmitted") {
                    await (0, handlers_1.handleContractEvent)(blockNumber, idx, evt);
                    try {
                        await detectAndHandlePspEvent(blockNumber, idx, evt);
                    }
                    catch (_a) { }
                }
                else if (method === "Instantiated") {
                    await (0, handlers_1.handleContractInstantiated)(blockNumber, idx, evt);
                }
            }
            else if (section === "nfts" || section === "uniques") {
                if (method === "Transferred") {
    ...
    // Process only signed extrinsics
    const extrinsics = (0, handlers_1.wrapExtrinsics)(block);
    for (const ext of extrinsics) {
        try {
            if (ext.extrinsic.isSigned) {
                (0, handlers_1.createExtrinsic)(blockNumber, ext);
                if (ext.extrinsic.method.section === "contracts") {
                    await (0, handlers_1.handleContractCall)(blockNumber, ext);
                }
            }
        }
        catch (error) {
...
async function handleNftsPalletCreated(blockNumber, eventRecord) {
    try {
        const [collectionId, creator, owner] = eventRecord.event.data;
        const id = collectionId.toString();
        let coll = await types_1.Psp34Collection.get(id);
        if (!coll) {
            coll = types_1.Psp34Collection.create({
                id,
                contractAddress: id,
                name: `Collection #${id}`,
                symbol: `NFT${id}`,
...
async function handleNftsPalletIssued(blockNumber, index, eventRecord) {
    try {
        ...
        // Ensure collection
        let coll = await types_1.Psp34Collection.get(collId);
        if (!coll) {
            await handleNftsPalletCreated(blockNumber, { event: { data: [collectionId, owner, owner] } });
            coll = await types_1.Psp34Collection.get(collId);
        }
        if (coll) {
        ...
        // Upsert token
        let token = await types_1.Psp34Token.get(tokenRecordId);
        if (!token) {
...
async function handleNftsPalletTransfer(blockNumber, index, eventRecord) {
    try {
        ...
        const tokenRecordId = `${collId}-${tokenId}`;
        // Update token owner
        const token = await types_1.Psp34Token.get(tokenRecordId);
        if (token) {
...
async function upsertNftAccount(collectionId, accountId, incoming) {
    const id = `${collectionId}-${accountId}`;
    let acct = await types_1.NftAccount.get(id);
    if (!acct) {
        acct = types_1.NftAccount.create({ id, collectionId, accountId, tokenCount: 0 });
    }
    acct.tokenCount = incoming ? acct.tokenCount + 1 : Math.max(0, acct.tokenCount - 1);
    await acct.save();
}
// ─── pallet-assets handlers ───────────────────────────────────────────────────
async function handleAssetCreated(blockNumber, eventRecord) {
    try {
        const [assetId, creator, owner] = eventRecord.event.data;
        const id = assetId.toString();
        let asset = await types_1.Asset.get(id);
        if (!asset) {
            // Create placeholder — NO RPC calls for maximum indexing speed.
            // Metadata (name, symbol, decimals) will be populated by the
            // assets.MetadataSet event which fires in the same block.
            asset = types_1.Asset.create({
                id,
                assetType: "Native",
...
async function handleAssetIssued(blockNumber, eventRecord) {
    try {
        ...
        if (!asset) {
            ...
            asset = await types_1.Asset.get(id);
        }
        if (asset) {
            asset.totalSupply = (asset.totalSupply || BigInt(0)) + issued;
            await asset.save();
        }
        // Create/update AssetAccount for the owner receiving the issued tokens
        const aaId = `${ownerStr}-${id}`;
        let aa = await types_1.AssetAccount.get(aaId);
        if (!aa) {
            aa = new types_1.AssetAccount(aaId, id, ownerStr, issued);
        }
        else {
            aa.balance = aa.balance + issued;
        }
        await aa.save();
    }
    catch (err) {
...
async function handleAssetMetadataSet(eventRecord) {
    try {
...
async function handleAssetDestroyed(eventRecord) {
    try {
        const [assetId] = eventRecord.event.data;
        const id = assetId.toString();
        const asset = await types_1.Asset.get(id);
        if (asset) {
...
async function detectAndHandlePspEvent(blockNumber, eventIndex, eventRecord) {
    try {
        ...
        const standard = (contract === null || contract === void 0 ? void 0 : contract.standard) || 'Unknown';
...
async function decodePsp22Event(blockNumber, index, contractAddress, rawData, eventRecord) {
    var _a;
    try {
        ...
        if (rawData.length < offset + 16)
            return;
        const valueBytes = rawData.slice(offset, offset + 16);
        const value = api.registry.createType('u128', valueBytes).toBigInt();
        if (!toAddr)
            return; // must have a recipient
        logger.info(`[PSP22] Transfer decoded: ${(_a = fromAddr === null || fromAddr === void 0 ? void 0 : fromAddr.slice(0, 8)) !== null && _a !== void 0 ? _a : 'mint'} -> ${toAddr.slice(0, 8)} value=${value}`);
        // Ensure token exists
        await (0, handlers_1.ensurePsp22Token)(contractAddress, blockNumber);
        // Build synthetic event record compatible with handlePsp22Transfer
        const syntheticEvent = {
            event: { data: [{ toString: () => contractAddress }, { toString: () => fromAddr !== null && fromAddr !== void 0 ? fromAddr : '' }, { toString: () => toAddr }, { toBigInt: () => value, toString: () => value.toString() }] }
        };
        await (0, handlers_1.handlePsp22Transfer)(blockNumber, index, syntheticEvent);
    }
    catch (err) {
...
async function decodePsp34Event(blockNumber, index, contractAddress, rawData, eventRecord) {
    var _a;
    try {
        ...
        let tokenId = '';
        if (idVariant === 0 && rawData.length > offset)
            tokenId = rawData[offset].toString();
        else if (idVariant === 1 && rawData.length >= offset + 2)
            tokenId = new DataView(rawData.buffer).getUint16(offset, true).toString();
        else if (idVariant === 2 && rawData.length >= offset + 4)
            tokenId = new DataView(rawData.buffer).getUint32(offset, true).toString();
        else if (idVariant === 3 && rawData.length >= offset + 8)
            tokenId = api.registry.createType('u64', rawData.slice(offset, offset + 8)).toString();
        else if (idVariant === 4 && rawData.length >= offset + 16)
            tokenId = api.registry.createType('u128', rawData.slice(offset, offset + 16)).toString();
        else
            tokenId = Buffer.from(rawData.slice(offset)).toString('hex');
        logger.info(`[PSP34] Transfer decoded: ${(_a = fromAddr === null || fromAddr === void 0 ? void 0 : fromAddr.slice(0, 8)) !== null && _a !== void 0 ? _a : 'mint'} -> ${toAddr.slice(0, 8)} tokenId=${tokenId}`);
        await (0, handlers_1.ensurePsp34Collection)(contractAddress, blockNumber);
        const syntheticEvent = {
            event: { data: [{ toString: () => contractAddress }, { toString: () => fromAddr !== null && fromAddr !== void 0 ? fromAddr : '' }, { toString: () => toAddr }, { toString: () => tokenId }] }
        };
        await (0, handlers_1.handlePsp34Transfer)(blockNumber, index, syntheticEvent);
    }
    catch (err) {
...
async function tryDetectFromData(blockNumber, index, contractAddress, rawData, eventRecord) {
